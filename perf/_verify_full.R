# Full-dataset behavior-preservation check (~357k rows, no subset). Captures a
# golden fingerprint of the postpro output on the COMPLETE imported dataset, then
# compares exact, unsorted, column-by-column (+ ts-scrubbed diagnostics). Run on
# `main` to capture, then on the optimized branch to compare.
#
#   Rscript perf/_verify_full.R          # capture if missing, else compare
#   Rscript perf/_verify_full.R reset    # force re-capture
...
fp <- list(ncol = ncol(out), nrow = nrow(out), colnames = names(out),
           cols = as.list(out), diag = scrub(diag))

golden_path <- here::here("data", ".autocode_bench", "golden_full.rds")
...
if (!file.exists(golden_path)) {
  ...
  cat(sprintf("CAPTURED golden_full: %d rows x %d cols\n", fp$nrow, fp$ncol))
} else {
  g <- readRDS(golden_path)
  ok <- TRUE
  chk <- function(label, a, b) { r <- identical(a, b); if (!r) { ok <<- FALSE; cat("  [DIFF] ", label, "\n") }; r }
  cat("=== VERIFY FULL vs golden (exact, unsorted) ===\n")
  chk("ncol", fp$ncol, g$ncol); chk("nrow", fp$nrow, g$nrow); chk("colnames", fp$colnames, g$colnames)
  if (identical(fp$colnames, g$colnames)) for (cn in fp$colnames) chk(paste0("col:", cn), fp$cols[[cn]], g$cols[[cn]])
  chk("diagnostics (ts-scrubbed)", fp$diag, g$diag)
  cat(sprintf("FULL rows=%d cols=%d -> %s\n", fp$nrow, fp$ncol, if (ok) "IDENTICAL" else "DIVERGED"))
}
